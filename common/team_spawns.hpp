...
class TeamSpawns {
  ...
  public:
    ...
    void load(const tmx::Map& map, unsigned teams, const CollisionWorld& walls,
        ...
        for (const auto& layer : map.getLayers()) {
            if (layer->getType() != tmx::Layer::Type::Object || layer->getName() != "Spawns")
                continue;
            for (const auto& object : layer->getLayerAs<tmx::ObjectGroup>().getObjects()) {
                auto p = object.getPosition();
                auto offset = layer->getOffset();
                sf::Vector2f world{(p.x - p.y) * map.getTileSize().x / (2.f * map.getTileSize().y) + offset.x,
                                   (p.x + p.y) * .5f + offset.y};
                if (walls.overlaps(world) || triggers.contains(world))
                    throw std::runtime_error("Unsafe spawn: " + object.getName());
                points.push_back(world);
            }
        }
        freeForAll_ = teams == 0;
        teams = std::max(1u, teams);
        if (teams > MAX_PLAYERS)
            throw std::runtime_error("Spawn team count exceeds maximum player slots");
        if (points.size() < teams)
            throw std::runtime_error("Spawns layer requires at least one safe spawn point per team: found " +
                                     std::to_string(points.size()) + " for " + std::to_string(teams) +
                                     " teams");
        groups_.assign(teams, {});
        cursors_.assign(teams, 0);
        partition(std::move(points), 0, teams);
    }
    const std::vector<std::vector<sf::Vector2f>>& groups() const {
        return groups_;
    }
        ...
        for (int ring = 0; ring <= 6; ++ring)
            for (std::size_t i = 0; i < group.size(); ++i) {
                ...
                for (int step = 0; step < (ring ? 16 : 1); ++step) {
                    ...
                    if (walls.overlaps(p) || triggers.contains(p))
                        continue;
                    bool occupied = false;
                    for (const auto& other : players)
                        if (other.connected && other.alive && (other.pos - p).length() < 21.f) {
                            occupied = true;
                            break;
                        }
                    if (!occupied) {
                        cursors_[team] = (index + 1) % group.size();
                        return p;
                    }
                }
            }
  ...
  private:
    void partition(std::vector<sf::Vector2f> points, unsigned first, unsigned teams) {
        ...
        std::sort(points.begin(), points.end(), [x](auto a, auto b) {
            return x ? (a.x == b.x ? a.y < b.y : a.x < b.x) : (a.y == b.y ? a.x < b.x : a.y < b.y);
        });
        unsigned left = teams / 2;
        auto split = points.size() * left / teams;
...
inline unsigned smallestTeam(const std::vector<PlayerState>& players, unsigned teams) {
    ...
    for (const auto& p : players)
        if (p.connected && p.team >= 0 && unsigned(p.team) < teams)
            ++count[p.team];
...
inline unsigned chooseTeam(const std::vector<PlayerState>& players, const ServerSettings& settings,
                           unsigned request) {
    const auto automatic = smallestTeam(players, settings.teams);
    if (request == 0 || request > settings.teams)
        return automatic;
    const auto preferred = request - 1;
    if (settings.honorTeamRequests)
        return preferred;
    unsigned smallestCount = 0, preferredCount = 0;
    for (const auto& player : players)
        if (player.connected) {
            if (player.team == static_cast<int>(automatic))
                ++smallestCount;
            if (player.team == static_cast<int>(preferred))
                ++preferredCount;
        }
