...
inline sf::Packet attackPacket(PlayerId player, const AttackRequest& request) {
    sf::Packet packet;
    packet << std::string(MSG_ATTACK) << player << request.sequence
           << static_cast<std::uint8_t>(request.kind) << request.aim.x << request.aim.y << request.ageMs;
    return packet;
}
inline bool readAttackRequest(sf::Packet& packet, PlayerId& player, AttackRequest& request) {
    std::uint8_t kind=0;
    if (!(packet >> player >> request.sequence >> kind >> request.aim.x >> request.aim.y >> request.ageMs)) return false;
...
public:
    bool accept(std::uint32_t sequence) {
        if (latest_ && !sequenceNewer(sequence,*latest_)) return false;
        latest_=sequence;
        return true;
    }
private:
...
public:
    ...
    void acknowledge(std::uint32_t sequence) {
        pending_.erase(std::remove_if(pending_.begin(),pending_.end(),[&](const auto& p){
            return p.request.sequence==sequence;
        }),pending_.end());
    }
    void clear() { pending_.clear(); }
private:
    struct Pending {AttackRequest request;std::uint32_t createdMs;};
