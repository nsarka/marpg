...
inline int parseBinding(std::string name) {
    ...
    if (name.size() == 1 && name[0] >= 'a' && name[0] <= 'z')
        return int(K::A) + name[0] - 'a';
    if (name.size() == 1 && name[0] >= '0' && name[0] <= '9')
        return int(K::Num0) + name[0] - '0';
    for (int i = 1; i <= 15; ++i)
        if (name == "f" + std::to_string(i))
            return int(K::F1) + i - 1;
    for (int i = 0; i <= 9; ++i)
        if (name == "numpad" + std::to_string(i))
            return int(K::Numpad0) + i;
    const std::pair<const char*, K> keys[] = {{"escape", K::Escape},
                                              {"tab", K::Tab},
                                              {"space", K::Space},
                                              {"enter", K::Enter},
                                              ...
                                              {"multiply", K::Multiply},
                                              {"divide", K::Divide}};
    for (auto [label, key] : keys)
        if (name == label)
            return int(key);
    const std::pair<const char*, sf::Mouse::Button> buttons[] = {{"mouse_left", sf::Mouse::Button::Left},
                                                                 {"mouse_right", sf::Mouse::Button::Right},
                                                                 {"mouse_middle", sf::Mouse::Button::Middle},
                                                                 {"mouse_x1", sf::Mouse::Button::Extra1},
                                                                 {"mouse_x2", sf::Mouse::Button::Extra2}};
    for (auto [label, button] : buttons)
        if (name == label)
            return mouseBinding(button);
    throw std::runtime_error("Unknown keybinding: " + name);
}
struct KeyBindings {
    const std::vector<int>& operator[](Action action) const {
