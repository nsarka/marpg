...
private:
    void initialize(const tmx::Map& map) {
        ...
        if(width_<=0 || height_<=0 || std::int64_t(width_)*height_>1000000)throw std::runtime_error("Invalid navigation dimensions");
        nodes_.resize(width_*height_);
        for(int y=0;y<height_;++y)for(int x=0;x<width_;++x) {
            auto& node=nodes_[y*width_+x];node.position=origin_+sf::Vector2f{float(x)*Spacing,float(y)*Spacing};
            node.walkable=safePoint(node.position);node.neighbors.fill(-1);
        }
        for(int y=0;y<height_;++y)for(int x=0;x<width_;++x) {
            auto& node=nodes_[y*width_+x];if(!node.walkable)continue;
            int slot=0;
            for(int dy=-1;dy<=1;++dy)for(int dx=-1;dx<=1;++dx) {
                ...
                if(nx>=0 && nx<width_ && ny>=0 && ny<height_) {
                    ...
                    if(nodes_[index].walkable && clear(node.position,nodes_[index].position))node.neighbors[slot]=index;
                }
                ++slot;
            }
        }
...
public:
    bool safePoint(sf::Vector2f p) const {
        ...
        for(auto offset:std::array<sf::Vector2f,5>{{{0,0},{10,0},{-10,0},{0,10},{0,-10}}})
            if(hazards_.contains(p+offset))return false;
        return true;
    }
    bool clear(sf::Vector2f from,sf::Vector2f to) const {
        if((walls_.move(from,to-from)-to).length()>.05f)return false;
    ...
    std::vector<sf::Vector2f> path(sf::Vector2f start,sf::Vector2f goal,const std::vector<sf::Vector2f>& people={}) const {
        if(!safePoint(goal))return {};
        const int source=nearest(start),target=nearest(goal);
        if(source<0 || target<0)return {};
        const float inf=std::numeric_limits<float>::infinity();
        std::vector<float> costs(nodes_.size(),inf);std::vector<int> parent(nodes_.size(),-1);
        using Entry=std::pair<float,int>;std::priority_queue<Entry,std::vector<Entry>,std::greater<Entry>> open;
        costs[source]=0;open.push({(nodes_[source].position-nodes_[target].position).length(),source});
        std::vector<bool> closed(nodes_.size());
        while(!open.empty()) {
            int current=open.top().second;open.pop();if(closed[current])continue;closed[current]=true;
            if(current==target) {
                std::vector<sf::Vector2f> result{goal};
                for(int n=target;n!=-1;n=parent[n])result.push_back(nodes_[n].position);
            ...
            for(int next:nodes_[current].neighbors) {
                ...
                for(auto p:people) {
                    ...
                    if(d<48)crowd+=(48-d)*3;
                }
                float cost=costs[current]+(nodes_[next].position-nodes_[current].position).length()+crowd;
...
private:
    int nearest(sf::Vector2f p) const {
        int best=-1;float distance=Spacing*Spacing*16;
        for(std::size_t i=0;i<nodes_.size();++i) {
            float d=(nodes_[i].position-p).lengthSquared();
            if(nodes_[i].walkable && d<distance && clear(p,nodes_[i].position)){best=i;distance=d;}
        }
        return best;
    }
    struct Node {sf::Vector2f position;bool walkable=false;std::array<int,8> neighbors;};
    const CollisionWorld& walls_;const TriggerSystem& hazards_;
    sf::Vector2f origin_{};int width_=0,height_=0;std::vector<Node> nodes_;
};
}
