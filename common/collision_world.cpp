...
void CollisionWorld::addPolygon(std::vector<sf::Vector2f> points) {
    if (points.size() < 3)
        throw std::runtime_error("Collision polygon needs at least three vertices");
    float area = 0;
    for (std::size_t i = 0; i < points.size(); ++i) {
        auto a = points[i], b = points[(i + 1) % points.size()];
        area += a.x * b.y - a.y * b.x;
    }
    if (std::abs(area) < Skin)
        throw std::runtime_error("Degenerate collision polygon");
    if (area < 0)
    ...
    for (const auto& layer : map.getLayers()) {
        ...
        for (std::size_t i = 0; i < tiles.size(); ++i) {
            ...
            for (const auto& set : map.getTilesets()) {
                if (set.hasTile(tiles[i].ID)) {
                    tile = set.getTile(tiles[i].ID);
                    tileOffset = {float(static_cast<std::int32_t>(set.getTileOffset().x)) +
                                      common::tileAlignmentCorrection(set, float(tile->imageSize.x),
                                                                      float(map.getTileSize().x)),
                                  float(static_cast<std::int32_t>(set.getTileOffset().y))};
                    break;
                }
            }
            if (!tile || tile->objectGroup.getObjects().empty())
                continue;
            ...
            const auto origin = tileImagePosition(
                tileToWorld(x, y, size), size, {float(tile->imageSize.x), float(tile->imageSize.y)},
            ...
            for (const auto& object : tile->objectGroup.getObjects()) {
                ...
                for (auto& p : points) {
                    ...
                    if (tiles[i].flipFlags & tmx::TileLayer::Horizontal)
                        p.x = width - p.x;
                    if (tiles[i].flipFlags & tmx::TileLayer::Vertical)
                        p.y = height - p.y;
                    p += origin;
                }
...
bool CollisionWorld::overlaps(sf::Vector2f position, float radius) const {
    for (const auto& polygon : polygons_) {
        ...
        for (auto plane : polygon.planes)
            if (dot(plane.normal, position) >= plane.distance + radius - Skin) {
                inside = false;
                break;
            }
        if (inside)
...
sf::Vector2f CollisionWorld::move(sf::Vector2f position, sf::Vector2f displacement, float radius,
    ...
    for (int iteration = 0; iteration < 8 && dot(displacement, displacement) > 1e-10f; ++iteration) {
        ...
        for (auto center : playerCenters) {
            ...
            if (b >= 0)
                continue; // Movement away from a touching player is free.
            ...
            if (t >= 0 && t <= first) {
                ...
                if (length > Skin) {
                    first = t;
                    hitNormal = offsetAtHit / length;
                    hit = true;
                }
            }
        }
        const float travel =
            hit ? std::max(0.f, first - Skin / std::sqrt(dot(displacement, displacement))) : first;
        ...
        if (intoWall < 0)
            displacement -= hitNormal * intoWall;
    }
    return position;
}
} // namespace common
