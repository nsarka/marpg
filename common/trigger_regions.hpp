...
namespace common {
inline std::vector<sf::Vector2f> triggerObjectPoints(const tmx::Map& map, const tmx::Layer& layer,
                                                     const tmx::Object& object) {
    std::vector<sf::Vector2f> points;
    if (object.getShape() == tmx::Object::Shape::Rectangle) {
        const auto b = object.getAABB();
        points = {{0, 0}, {b.width, 0}, {b.width, b.height}, {0, b.height}};
    } else if (object.getShape() == tmx::Object::Shape::Polygon) {
        for (auto p : object.getPoints())
            points.push_back({p.x, p.y});
    } else
        throw std::runtime_error("Trigger must be a rectangle or convex polygon: " + object.getName());
    const auto origin = object.getPosition();
    ...
    for (auto& point : points) {
        ...
        point = {(x - y) * map.getTileSize().x / (2.f * map.getTileSize().y) + map.getTileSize().x * .5f +
                     offset.x,
                 (x + y) * .5f + offset.y};
    }
    return points;
...
struct TriggerRegion {
    ...
    bool contains(sf::Vector2f point) const {
        return shape.overlaps(point, 0.f);
    }
};
class TriggerRegions {
  ...
  public:
    std::shared_ptr<const TriggerRegion> add(std::vector<sf::Vector2f> points, sf::Color color) {
        ...
        region->shape.addPolygon(std::move(points));
        region->color = color;
        regions_.erase(
            std::remove_if(regions_.begin(), regions_.end(), [](const auto& r) { return r.expired(); }),
            regions_.end());
