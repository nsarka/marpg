  ...
  public:
    void observe(const std::vector<PlayerState*>& players, const TriggerSystem& hazards) {
        for (std::size_t victim = 0; victim < players.size() && victim < seen_.size(); ++victim) {
            ...
            if (!changed || !player.connected || player.alive || player.health > 0 ||
                player.damageEvents.empty())
                continue;
            ++player.deaths;
            ...
            event.victim = static_cast<PlayerId>(victim);
            event.victimName = player.name;
            ...
            if (damage.source >= 0 && std::size_t(damage.source) < players.size()) {
                ...
                event.killerTeam = killer.team;
                event.cause = killer.lastAttack == AttackKind::Lightning   ? KillCause::Lightning
                              : killer.lastAttack == AttackKind::Explosion ? KillCause::Spell
                              : killer.lastAttack == AttackKind::Light     ? KillCause::Light
                              : killer.lastAttack == AttackKind::Heavy     ? KillCause::Heavy
                                                                           : KillCause::Hit;
            } else {
                event.killer = -1;
                event.killerName = "WORLD";
                event.cause = hazards.hasFloor(player.pos) ? KillCause::Floor : KillCause::Bounds;
            }
            if (events_.size() == KillHistorySize)
                events_.erase(events_.begin());
            events_.push_back(std::move(event));
        }
    }
    const std::vector<KillEvent>& events() const {
        return events_;
    }
