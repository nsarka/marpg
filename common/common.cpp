...
bool readInputCmd(sf::Packet& packet, common::PlayerId& id, common::InputCommand& cmd) {
    ...
    if (!(packet >> id >> cmd.sequence >> moveX >> moveY >> cmd.sprint >> cmd.lightHeld >> cmd.lightPressed >> cmd.lightReleased >> cmd.heavyHeld >> cmd.heavyPressed >> cmd.heavyReleased >> cmd.aim.x >> cmd.aim.y)) {
        return false;
    }
