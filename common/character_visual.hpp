...
inline bool isPlayerAnimation(const tmx::Tileset& tileset, std::uint32_t gid) {
    if (const auto* tile = tileset.getTile(gid))
        for (const auto& property : tile->properties)
            if (property.getName() == "player_animation" &&
                property.getType() == tmx::Property::Type::Boolean)
                return property.getBoolValue();
    return false;
}
inline void applyCharacterTileLayout(sf::Vector2f frameSize, sf::Vector2f mapTileSize, sf::Vector2f& drawSize,
                                     sf::Vector2f& offset) {
