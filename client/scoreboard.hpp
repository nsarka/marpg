...
inline void drawScoreboard(sf::RenderTarget& target, const sf::Font& font,
    ...
    for (std::size_t i = 0; i < players.size(); ++i)
        if (players[i].connected) {
            if (settings.teams == 0)
                teams[0].push_back(i);
            else if (players[i].team >= 0 && std::size_t(players[i].team) < teams.size())
    ...
    sf::View view(sf::FloatRect({0, 0}, oldView.getSize() / scale));
    target.setView(view);
    ...
    auto text = [&](sf::String value, float x, float y, unsigned size, sf::Color color,
                    float maxWidth = 1000.f) {
        sf::Text label(font, value, uiFontSize(size));
        label.setFillColor(color);
        label.setStyle(sf::Text::Bold);
        while (label.getLocalBounds().size.x > maxWidth && value.getSize() > 1) {
            value.erase(value.getSize() - 1, 1);
            label.setString(value + sf::String(U'…'));
        }
    ...
    text(sf::String::fromUtf8(serverName.begin(), serverName.end()), left + 24, top + 18, 24,
         sf::Color::White, 852);
    std::array<float, 2> y{top + 68, top + 68};
    for (std::size_t t = 0; t < teams.size(); ++t) {
        const auto col = t % 2;
        const float x = left + 24 + col * 444;
        auto color = common::teamColor(t, settings.teams);
        color.a = 255;
        ...
        text("PING", x + 365, y[col], 13, sf::Color(165, 175, 190));
        y[col] += 30;
        for (auto id : teams[t]) {
            const auto& p = players[id];
            if (id == local)
                box(x - 6, y[col] - 1, 420, 32, sf::Color(255, 255, 255, 25));
            text(sf::String::fromUtf8(p.name.begin(), p.name.end()) + (p.alive ? "" : " *DEAD*"), x, y[col],
                 17, color, 226);
            text(std::to_string(p.kills), x + 252, y[col], 17, sf::Color::White);
            text(std::to_string(p.deaths), x + 313, y[col], 17, sf::Color::White);
            text(p.pingMs < 0 ? "—" : std::to_string(p.pingMs) + " ms", x + 360, y[col], 13,
                 sf::Color(185, 195, 205), 48);
            y[col] += 32;
        }
        y[col] += 22;
    }
