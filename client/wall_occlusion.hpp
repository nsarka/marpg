    ...
    void addLayer(const tmx::Map& map, std::size_t layerIndex) {
        ...
        for (std::size_t i = 0; i < tiles.size(); ++i) {
            ...
            for (const auto& set : map.getTilesets()) {
                if (tiles[i].ID && set.hasTile(tiles[i].ID)) {
            ...
            if (!tile || tile->objectGroup.getObjects().empty())
                continue;
            const bool hasSolid =
                std::any_of(tile->objectGroup.getObjects().begin(), tile->objectGroup.getObjects().end(),
                            [](const auto& object) { return object.getClass() != "DamageTrigger"; });
            if (!hasSolid)
                continue;
            sf::Image image;
            if (!image.loadFromFile(tile->imagePath))
                throw std::runtime_error("Cannot load wall mask image");
            ...
            for (unsigned col = 0; col < size.x; ++col) {
                ...
                while (hull.size() > 1) {
                    ...
                    if ((b.x - a.x) * (p.y - b.y) - (b.y - a.y) * (p.x - b.x) < 0)
                        break;
                    hull.pop_back();
                }
                hull.push_back(p);
            ...
            for (unsigned col = 0; col < size.x; ++col) {
                while (segment + 2 < hull.size() && col > hull[segment + 1].x)
                    ++segment;
