  ...
  public:
    ...
    void observe(const std::vector<common::PlayerState>& players, common::PlayerId local,
        ...
        for (std::size_t victim = 0; victim < players.size() && victim < seen_.size(); ++victim) {
            ...
            for (const auto& event : players[victim].damageEvents) {
                ...
                numbers_.push_back({event.amount,
                                    event.contact + sf::Vector2f{x(random_), -14.f + y(random_)},
                                    victim == local});
            }
        }
    }
    void update(float dt) {
        for (auto& number : numbers_)
            number.age += std::max(0.f, dt);
        numbers_.erase(std::remove_if(numbers_.begin(), numbers_.end(),
                                      [](const auto& number) { return number.age >= Lifetime; }),
                       numbers_.end());
    }
    ...
    static sf::Color color(const Number& number) {
        const auto alpha =
            static_cast<std::uint8_t>(255.f * std::clamp(1.f - number.age / Lifetime, 0.f, 1.f));
        return number.incoming ? sf::Color(255, 75, 80, alpha) : sf::Color(255, 255, 255, alpha);
    }
    static std::string label(const Number& number) {
        return "-" + std::to_string(number.amount);
    }
    void draw(sf::RenderTarget& target, const sf::Font& font) const {
        for (const auto& number : numbers_) {
            ...
            text.setOutlineColor(sf::Color(15, 18, 22, tint.a));
            text.setOutlineThickness(1.5f);
            const auto bounds = text.getLocalBounds();
            text.setOrigin({bounds.position.x + bounds.size.x * .5f, bounds.position.y + bounds.size.y});
            text.setPosition(position(number));
            target.draw(text);
        }
    }
    const std::vector<Number>& numbers() const {
        return numbers_;
    }
