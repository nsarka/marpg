...
inline float attackSecondsRemaining(const common::PlayerState& player,common::AttackKind ability=common::AttackKind::None,const common::ServerSettings& settings=common::ServerSettings{}) {
    if(ability==common::AttackKind::None)ability=player.combatDebug.attack;
    float remaining=player.stunTicks*common::TICK_DT;
    if(ability==common::AttackKind::Explosion)remaining=std::max(remaining,player.explosionCooldown*common::TICK_DT);
    ...
    const bool spell=kind==common::AttackKind::Explosion || kind==common::AttackKind::Lightning;
...
inline void drawAttackHud(sf::RenderTarget& target,const sf::Font& font,const common::PlayerState& player,const common::KeyBindings& bindings = {},const common::ServerSettings& settings=common::ServerSettings{}) {
    ...
    for(unsigned i=0;i<4;++i) {
        ...
        const sf::String title=sf::String(presentation.name)+" ("+abilityBindingLabel(bindings.actions[presentation.bindingAction])+")";
        ...
        if(remaining>0) {
            ...
            if(ability==common::AttackKind::Explosion || ability==common::AttackKind::Lightning)
                duration=std::max(duration,common::attackDescription(ability, settings).recoveryTicks*common::TICK_DT);
