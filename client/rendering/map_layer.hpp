  ...
  public:
    MapLayer(const tmx::Map& map, std::size_t idx, TileLighting* lighting = nullptr)
        ...
        if (tileLighting.enabled) {
            ...
            if (!m_lightingShader->loadFromFile("../shaders/tile_lighting.vert",
                                                "../shaders/tile_lighting.frag"))
        ...
        m_chunkSize.x =
            std::floor(m_chunkSize.x / static_cast<float>(tileSize.x)) * static_cast<float>(tileSize.x);
        m_chunkSize.y =
            std::floor(m_chunkSize.y / static_cast<float>(tileSize.y)) * static_cast<float>(tileSize.y);
        ...
        m_globalBounds.position = {-static_cast<float>(m_mapTileCount.y) * halfW, 0.f};
        m_globalBounds.size = {mapPixelWidth, mapPixelHeight};
    }
        ...
        for (auto& chunk : m_chunks) {
            ...
            for (auto& animation : chunk->getActiveAnimations()) {
                ...
                for (const auto& frame : frames)
                    cycle += std::max<std::int64_t>(1, frame.duration) * 1000;
                const auto time = (animation.currentTime.asMicroseconds() +
                                   std::max<std::int64_t>(0, elapsed.asMicroseconds())) %
                                  cycle;
                ...
                const auto x = static_cast<std::int32_t>(animation.tileCoords.x),
                           y = static_cast<std::int32_t>(animation.tileCoords.y);
                if (getTile(x, y).ID != id) {
                    tmx::TileLayer::Tile tile;
                    tile.ID = id;
                    tile.flipFlags = animation.flipFlags;
