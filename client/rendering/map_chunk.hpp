  ...
  private:
    class ChunkArray final : public sf::Drawable {
      public:
        ...
        explicit ChunkArray(const sf::Texture& texture, const sf::Texture* normal, const sf::Texture* height,
                            unsigned stair, std::uint8_t flips, std::shared_ptr<sf::Shader> shader,
                            TileLighting& lighting)
            : tileLighting(lighting), m_texture(texture), m_normal(normal), m_height(height), m_stair(stair),
              m_flips(flips), m_shader(std::move(shader)) {}
      ...
      private:
        ...
        void draw(sf::RenderTarget& rt, sf::RenderStates states) const override {
            ...
            if (m_normal && tileLighting.enabled) {
                ...
                const auto local =
                    states.transform.getInverse().transformPoint({tileLighting.sun.x, tileLighting.sun.y});
                ...
                if (m_height)
                    shader.setUniform("heightMap", *m_height);
                shader.setUniform("sunPosition", sf::Glsl::Vec3(local.x, local.y, tileLighting.sun.z));
                shader.setUniform("flip", sf::Glsl::Vec3((m_flips & tmx::TileLayer::Horizontal) ? 1.f : 0.f,
                                                         (m_flips & tmx::TileLayer::Vertical) ? 1.f : 0.f,
                                                         (m_flips & tmx::TileLayer::Diagonal) ? 1.f : 0.f));
                shader.setUniform("ambient", tileLighting.ambient);
                shader.setUniform("intensity", tileLighting.intensity);
                ...
                if (tileLighting.stairMap)
                    shader.setUniform("stairMap", *tileLighting.stairMap);
                if (tileLighting.shadowMap) {
                    shader.setUniform("shadowMap", *tileLighting.shadowMap);
                    const auto origin =
                        states.transform.getInverse().transformPoint(tileLighting.shadowOrigin);
    ...
    static void flipY(sf::Vector2f* v0, sf::Vector2f* v1, sf::Vector2f* v2, sf::Vector2f* v3,
    ...
    static void flipX(sf::Vector2f* v0, sf::Vector2f* v1, sf::Vector2f* v2, sf::Vector2f* v3,
                      sf::Vector2f* v4, sf::Vector2f* v5) {
        const sf::Vector2f tmp0 = *v0;
        v0->x = v1->x;
        v3->x = v1->x;
    ...
    static void flipD(sf::Vector2f* v0, sf::Vector2f* v1, sf::Vector2f* v2, sf::Vector2f* v3,
                      sf::Vector2f* v4, sf::Vector2f* v5) {
        const sf::Vector2f tmp2 = *v2;
        *v2 = *v4;
        *v4 = tmp2;
    ...
    static void doFlips(std::uint8_t bits, sf::Vector2f* v0, sf::Vector2f* v1, sf::Vector2f* v2,
    ...
    ChunkArray& getOrCreateChunkArray(const TileVisual& visual, std::uint8_t flips) {
        ...
        auto array = std::make_unique<ChunkArray>(*visual.texture, visual.normal, visual.height, visual.stair,
                                                  flips, visual.lightingShader, tileLighting);
        auto* raw = array.get();
    ...
    void generateTiles(bool registerAnimation = false) {
        ...
        for (std::uint32_t localY = 0; localY < m_chunkTileCount.y; ++localY) {
            for (std::uint32_t localX = 0; localX < m_chunkTileCount.x; ++localX) {
                ...
                const auto worldTopLeft = common::tileImagePosition(isoBase, sf::Vector2f(m_mapTileSize),
                                                                    visual.drawSize, visual.offset);

                if (registerAnimation && visual.height)
                    tileLighting.heightTiles.push_back({worldTopLeft + m_layerOffset,
                                                        visual.height->copyToImage(), tile.flipFlags,
                                                        visual.stair});
                ...
                Tile quad = {
                    ...
                    sf::Vertex{localTopLeft + sf::Vector2f{visual.drawSize.x, 0.f}, m_chunkColors[idx],
                               visual.texTopLeft + sf::Vector2f{visual.texSize.x, 0.f}},
                    sf::Vertex{localTopLeft + visual.drawSize, m_chunkColors[idx],
                               visual.texTopLeft + visual.texSize},

                    sf::Vertex{localTopLeft, m_chunkColors[idx], visual.texTopLeft},
                    sf::Vertex{localTopLeft + visual.drawSize, m_chunkColors[idx],
                               visual.texTopLeft + visual.texSize},
                    sf::Vertex{localTopLeft + sf::Vector2f{0.f, visual.drawSize.y}, m_chunkColors[idx],
