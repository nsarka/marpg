...
class TileAssets {
    ...
    void registerAtlasTileset(const tmx::Tileset& tileset) {
        ...
        const std::uint32_t columns =
            tileset.getColumnCount() > 0 ? tileset.getColumnCount() : (texture.getSize().x / tileSize.x);
        ...
        for (std::uint32_t localID = 0; localID < tileset.getTileCount(); ++localID) {
            ...
            TileVisual visual;
            visual.offset = {float(static_cast<std::int32_t>(tileset.getTileOffset().x)),
                             float(static_cast<std::int32_t>(tileset.getTileOffset().y))};
            visual.textureKey = atlasPath;
            visual.texture = &texture;
            visual.texTopLeft = {static_cast<float>(margin + col * (tileSize.x + spacing)),
                                 static_cast<float>(margin + row * (tileSize.y + spacing))};
            visual.texSize = {static_cast<float>(tileSize.x), static_cast<float>(tileSize.y)};
            visual.drawSize = visual.texSize;
            visual.offset.x +=
                common::tileAlignmentCorrection(tileset, visual.drawSize.x, float(m_mapTileSize.x));
    ...
    void registerCollectionTileset(const tmx::Tileset& tileset) {
        for (const auto& tile : tileset.getTiles()) {
            ...
            visual.stair = stem == "stairs_E"              ? 1
                           : stem == "stairs_S"            ? 2
                           : stem == "stairsCornerOuter_S" ? 3
                                                           : 0;
            ...
            if (lighting_->enabled && std::filesystem::exists(albedo) && std::filesystem::exists(normal)) {
                ...
                if (std::filesystem::exists(height))
                    visual.height = &loadTextureOrFallback(height.string(), nullptr);
                visual.lightingShader = m_lightingShader;
            }
            visual.texTopLeft = {0.f, 0.f};
            visual.texSize = {static_cast<float>(width), static_cast<float>(height)};
            visual.drawSize = visual.texSize;
            visual.offset.x +=
                common::tileAlignmentCorrection(tileset, visual.drawSize.x, float(m_mapTileSize.x));
            if (common::isPlayerAnimation(tileset, gid))
  ...
  public:
    void configure(sf::Vector2u size, std::shared_ptr<sf::Shader> shader, TileLighting& lighting) {
        lighting_ = &lighting;
        m_mapTileSize = size;
        m_lightingShader = std::move(shader);
    }
