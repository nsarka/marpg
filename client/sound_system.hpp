...
public:
    explicit SoundSystem(const std::filesystem::path& directory) {
        ...
        for (std::filesystem::directory_iterator it(directory,error),end;!error && it!=end;it.increment(error))
            if (it->path().extension()==".wav") files.push_back(it->path());
        if (error) std::cerr<<"Sound pack unavailable: "<<error.message()<<'\n';
        std::sort(files.begin(),files.end());
        for (const auto& path:files) {
            const auto category=soundCategory(path.filename().string());
            if (!category) continue;
            auto buffer=std::make_unique<sf::SoundBuffer>();
            if (buffer->loadFromFile(path)) banks_[index(*category)].buffers.push_back(std::move(buffer));
            else std::cerr<<"Cannot load sound: "<<path<<'\n';
        }
        for (const auto& bank:banks_) if (bank.buffers.empty())
            std::cerr<<"Sound effect group has no playable variants\n";
    }
    std::size_t variantCount(SoundEffect effect) const {return banks_[index(effect)].buffers.size();}
    void observe(const std::vector<common::PlayerState>& players, sf::Vector2f listener) {
        ...
        for (const auto& cue:events_.observe(players)) play(cue,listener);
        for (auto& voice:voices_) mix(voice,listener);
    }
    void play(SoundCue cue,sf::Vector2f listener) {
        if (common::distance(cue.position,listener)>=900.f) return;
        auto& bank=banks_[index(cue.effect)];
        if (bank.buffers.empty()) return;
        std::size_t choice=0;
        if (bank.buffers.size()>1) {
            const bool excludeLast=bank.last.has_value();
            choice=std::uniform_int_distribution<std::size_t>(0,bank.buffers.size()-1-excludeLast)(random_);
            if (excludeLast && choice>=*bank.last) ++choice;
        }
        bank.last=choice;
...
private:
    static std::size_t index(SoundEffect effect) {return static_cast<std::size_t>(effect);}
    struct Bank {std::vector<std::unique_ptr<sf::SoundBuffer>> buffers;std::optional<std::size_t> last;};
    struct Voice {std::unique_ptr<sf::Sound> sound;sf::Vector2f position;float volume;};
