...
ResourceManager::ResourceManager(common::Logger& logger) : m_logger(logger) {}
...
bool ResourceManager::loadFragmentShader(const std::string& id, const std::filesystem::path& path) {
    sf::Shader shader;
...
const ResourceManager::AnimSet&
ResourceManager::CharacterAnimations::get(common::CharacterAnimation animation) const {
    ...
    if (!texture->loadFromFile(directory / (std::string(definition.name) + ".png")))
        throw std::runtime_error("Cannot load character animation: " + std::string(definition.name));
    if (texture->getSize() != sf::Vector2u{1920, 1024})
        throw std::runtime_error("Expected 15 by 8 grid of 128-pixel frames");
    constexpr int rows[8] = {3, 4, 5, 6, 7, 0, 1, 2};
    for (std::size_t facing = 0; facing < 8; ++facing) {
        auto& clip = set.byFacing[facing];
        clip.texture = texture;
        clip.looping = definition.looping;
        for (int frame = 0; frame < 15; ++frame)
            clip.frames.push_back({{{frame * 128, rows[facing] * 128}, {128, 128}}, definition.frameSeconds});
    }
    set.loaded = true;
    return set;
}
...
bool ResourceManager::loadFantasyCharacter(const std::string& id, const std::filesystem::path& assetRoot) {
    try {
        ...
        for (const auto& definition : common::CharacterAnimations)
            if (!std::filesystem::is_regular_file(assetRoot / (std::string(definition.name) + ".png")))
                throw std::runtime_error("Missing animation: " + std::string(definition.name));
...
ResourceManager::CharacterAnimations& ResourceManager::getCharacterAnimations(const std::string& id) {
    return m_characterAnimations.at(id);
}
