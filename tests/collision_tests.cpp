...
int main(int argc, char** argv) {
    {
        const auto file =
            ...
            ("marpg-flips-" + std::to_string(std::chrono::steady_clock::now().time_since_epoch().count()) +
             ".tmx");
        for (unsigned flags = 0; flags < 8; ++flags) {
            const std::uint32_t gid = 1u | ((flags & 1) ? 0x80000000u : 0) | ((flags & 2) ? 0x40000000u : 0) |
                                      ((flags & 4) ? 0x20000000u : 0);
            std::ofstream out(file);
            out << "<map version='1.10' orientation='isometric' width='1' height='1' tilewidth='256' "
                   "tileheight='128' infinite='0'>"
                   "<tileset firstgid='1' name='Flip test' tilewidth='100' tileheight='200' tilecount='1' "
                   "columns='0'>"
                   "<tile id='0'><image source='unused.png' width='100' height='200'/><objectgroup>"
                   "<object id='1' x='20' y='60' width='20' height='40'/>"
                   "<object id='2' class='DamageTrigger' x='20' y='60'><polygon points='0,0 20,0 20,40 "
                   "0,40'/></object>"
                   "</objectgroup></tile></tileset><layer name='Walls' width='1' height='1'><data "
                   "encoding='csv'>"
                << gid << "</data></layer></map>";
            out.close();
            ...
            for (bool triggers : {false, true}) {
                common::CollisionWorld flipped;
                flipped.load(file.string(), triggers);
                check(flipped.size() == 1 && flipped.overlaps(center, 0),
                      "Flipped polygon not at rendered tile position");
