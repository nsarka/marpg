...
int main(int argc, char** argv) {
    ...
    const auto id = connection.connectToServer("127.0.0.1", "Config probe",
    ...
    if (argc == 3) {
        ...
        std::vector<common::PlayerState> states;
        std::vector<common::PlayerId> joined;
        loading.finish(states, joined);
        if (connection.shuttingDown() || !connection.hasWorldSnapshot() || states.size() <= id ||
            !states[id].connected)
            return 5;
        auto playerId = id;
        common::InputCommand input;
        input.sequence = 1;
        connection.sendInput(playerId, input);
    ...
    if (s.port != std::stoi(argv[1]) || s.teams != 3 || s.slots != 6 || s.bots != 0 ||
        s.lightDamageMax != 27 || s.heavyDamageMax != 41 || s.triggerDamage != 7 || s.boundsDamage != 9 ||
        s.triggerIntervalSeconds != 1.0 || s.boundsIntervalSeconds != .25)
        return 3;
    if (common::attackDescription(common::AttackKind::Heavy, s).damageMax != 41)
        return 4;
