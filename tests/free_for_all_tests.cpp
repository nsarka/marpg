...
int main(int argc, char** argv) {
    ...
    sf::Packet maxPacket;
    common::writeSettings(maxPacket, maxTeams);
    check(common::readSettings(maxPacket, received) && received.teams == 32,
          "32 teams did not survive network settings");
    ...
    try {
        ...
        rejected = std::string(e.what()).find("0-32") != std::string::npos;
    }
    std::filesystem::remove(temp);
    check(rejected, "Teams error does not explain valid range");
    rulesUnderTest = settings;
    common::CollisionWorld walls;
    walls.load(argv[1]);
    common::TriggerSystem hazards;
    hazards.load(argv[1], rulesUnderTest);
    common::TeamSpawns spawns;
    spawns.load(argv[1], 0, walls, hazards);
    check(spawns.groups().size() == 1, "FFA spawns were split into teams");
    auto spawn = spawns.choose(static_cast<unsigned>(-1), {}, walls, hazards);
    check(spawn.has_value(), "Teamless player cannot spawn");
    ...
    for (auto kind : {common::AttackKind::Light, common::AttackKind::Heavy, common::AttackKind::Explosion,
        ...
        auto aim = (kind == common::AttackKind::Light || kind == common::AttackKind::Heavy)
                       ? sf::Vector2f{1, 0}
                       : b.pos;
        check(common::startAttack(a, combat, kind, aim, rulesUnderTest), "Cannot attack in FFA");
        for (int tick = 0; tick < 500 && b.health == 100; ++tick)
            common::updateAttack(a, combat, {&a, &b}, empty, {}, rulesUnderTest);
        check(b.health < 100, "FFA damage blocked by friendly-fire setting");
    }
    common::Navigation nav(argv[1], walls, hazards);
    common::BotAI ai(nav, walls, 42, rulesUnderTest);
    common::PlayerState bot, enemy;
    bot.connected = enemy.connected = true;
    bot.team = enemy.team = -1;
    bot.pos = *spawn;
    enemy.pos = *spawn + sf::Vector2f{30, 0};
    common::CombatState combat;
    ai.update(0, bot, combat, {&bot, &enemy});
    check(ai.target(0) == 1, "FFA bot ignored teamless opponent");
}
