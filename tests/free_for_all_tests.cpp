...
int main(int argc,char** argv) {
    ...
    common::CollisionWorld walls;walls.load(argv[1]);common::TriggerSystem hazards;hazards.load(argv[1]);
    ...
    for(auto kind:{common::AttackKind::Jab,common::AttackKind::Hook,common::AttackKind::Uppercut,common::AttackKind::Lightning}) {
        ...
        auto aim=(kind==common::AttackKind::Jab || kind==common::AttackKind::Hook)?sf::Vector2f{1,0}:b.pos;
        check(common::startAttack(a,combat,kind,aim),"Cannot attack in FFA");
        for(int tick=0;tick<500 && b.health==100;++tick)common::updateAttack(a,combat,{&a,&b},empty);
        check(b.health<100,"FFA damage blocked by friendly-fire setting");
    }
    common::Navigation nav(argv[1],walls,hazards);common::BotAI ai(nav,walls,42);
