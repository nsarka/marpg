...
int main(int argc, char** argv) {
    ...
    for (std::size_t i = 0; i < counts.size(); ++i)
        check(sounds.variantCount(static_cast<SoundEffect>(i)) == counts[i],
              "All applicable WAV variants must decode");
    check(sounds.variantCount(SoundEffect::Cast) > 0 && sounds.variantCount(SoundEffect::Blast) > 0,
          "Spell audio assets missing");
    ...
    player.lastAttack = common::AttackKind::Light;
    ...
    player.lastAttack = common::AttackKind::Explosion;
    ++player.attackSequence;
    cues = events.observe(players);
    check(cues.size() == 1 && cues[0].effect == SoundEffect::Cast, "Cast sound missing");
    ++player.spellSequence;
    player.spellPosition = {40, 50};
    cues = events.observe(players);
    check(cues.size() == 1 && cues[0].effect == SoundEffect::Blast &&
              cues[0].position == player.spellPosition,
          "AoE sound missing");
    check(events.observe(players).empty(), "Spell audio replayed");
    player.connected = false;
    events.observe(players);
    player.connected = true;
