...
int main(int argc, char** argv) {
    check(argc >= 2, "Root required");
    const bool gallery = argc > 2;
    const auto path =
        (std::filesystem::path(argv[1]) / (gallery ? "assets/tiled/demo.tmx" : "assets/tiled/arena.tmx"))
            .string();
    tmx::Map map;
    ...
    for (const auto& tileset : map.getTilesets()) {
        if (tileset.getName() == "Gallery floor" || tileset.getName() == "Fantasy pack gallery" ||
            ...
            check(common::tileAlignmentCorrection(tileset, 256, 128) == 64.f,
                  "Tiled alignment correction missing");
            // Pivot y=256*(1-.18) must land at the map cell center, within pixel rounding.
            check(std::abs(64.f - 256.f + 14.f + 256.f * (1.f - .18f) - 32.f) < .5f,
    ...
    if (gallery) {
        ...
        for (const auto& set : map.getTilesets()) {
            if (set.getImagePath().find("/Characters/") == std::string::npos)
                continue;
            ++characterSets;
            for (unsigned frame = 0; frame < set.getTileCount(); ++frame)
                check(common::isPlayerAnimation(set, set.getFirstGID() + frame),
                      "Character frame lacks player_animation property");
        }
        ...
        check(64.f - size.y + offset.y + size.y * common::CharacterFeetY == 32.f,
              "Gallery character feet not at cell center");
    }
    common::CollisionWorld walls;
    walls.load(path);
    if (!gallery)
        check(walls.size() > 20, "Fantasy collision missing");
    common::TriggerSystem triggers;
    triggers.load(path);
    common::TeamSpawns spawns;
    spawns.load(path, 2, walls, triggers);
    if (!gallery)
        for (int y : {2, 3, 20, 21})
            for (int x = (y < 4 ? 2 : 17); x < (y < 4 ? 7 : 22); ++x) {
    ...
    for (std::size_t i = 0; i < map.getLayers().size(); ++i) {
        if (map.getLayers()[i]->getName() == "Floor")
            floorIndex = i;
        if (map.getLayers()[i]->getName() == "Walls")
            wallIndex = i;
    }
    MapLayer floor(map, floorIndex), objects(map, wallIndex);
    WallOcclusion masks(map, wallIndex);
    sf::RenderTexture target({1100, 650});
    target.setView(sf::View(sf::FloatRect({-1550, -50}, {3228, 1750})));
    if (gallery)
        target.setView(sf::View(sf::FloatRect({-900, 600}, {2200, 1300})));
    if (gallery) {
        ...
        for (std::size_t i = 0; i < tiles.size(); ++i) {
            ...
            if (found == map.getAnimatedTiles().end() || found->second.animation.frames.size() < 2)
                continue;
            ...
            check(objects.getTile(x, y).ID == frames[1].tileID,
                  "World animation did not advance at frame boundary");
            int cycle = 0;
    ...
    target.clear(sf::Color(35, 40, 35));
    target.draw(floor);
    target.draw(objects);
    target.display();
    check(target.getTexture().copyToImage().saveToFile(
              std::filesystem::temp_directory_path() /
              (gallery ? "marpg-gallery.png" : "marpg-fantasy-map.png")),
          "Preview failed");
}
