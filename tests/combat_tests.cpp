...
common::ServerSettings unstunnedRules() {
    common::ServerSettings rules;rules.damageStunSeconds=0;rules.lightConeDegrees=rules.heavyConeDegrees=90;rules.lightRange=70;rules.heavyRange=80;return rules;
}
int main(){
    common::ServerSettings rulesUnderTest;
    rulesUnderTest = common::ServerSettings{};
    check(common::inAttackArc({60,90},{1,0},140,common::AttackKind::Light,rulesUnderTest),"120-degree light should include a 56-degree target");
    check(!common::inAttackArc({60,110},{1,0},140,common::AttackKind::Light,rulesUnderTest),"Light should exclude targets beyond 60 degrees");
    check(common::inAttackArc({150,20},{1,0},160,common::AttackKind::Heavy,rulesUnderTest),"Extended narrow heavy should hit");
    check(!common::inAttackArc({150,30},{1,0},160,common::AttackKind::Heavy,rulesUnderTest),"Heavy should exclude targets beyond 10 degrees");
    ...
    rulesUnderTest = unstunnedRules();
    {
        ...
        for(auto kind:{common::AttackKind::Light,common::AttackKind::Heavy,common::AttackKind::Explosion,common::AttackKind::Lightning}) {
            ...
            windup.bufferedAttack=common::AttackKind::Heavy;windup.bufferedTicks=10;
            hit.age=common::attackDescription(hit.attack,rulesUnderTest).startupTicks-1;
            ...
            common::startAttack(victim,windup,kind,{0,0},rulesUnderTest);
            windup.age=common::attackDescription(kind,rulesUnderTest).startupTicks;
            check(!common::interruptWindup(windup,rulesUnderTest) && windup.attack==kind,"Active attacks must not be interrupted");
            windup.age+=common::attackDescription(kind,rulesUnderTest).activeTicks;
        ...
        cast.age=common::attackDescription(cast.attack,rulesUnderTest).startupTicks-1;
    ...
    }
    {
        ...
        rules.lightDamageMin=3;rules.lightDamage=9;rules.lightConeDegrees=30;rules.heavyConeDegrees=180;
        rulesUnderTest = rules;
        check(!common::inAttackArc({30,20},{1,0},70,common::AttackKind::Light,rulesUnderTest),"Narrow light cone ignored");
        check(common::inAttackArc({30,20},{1,0},80,common::AttackKind::Heavy,rulesUnderTest),"Wide heavy cone ignored");
        ...
        for(int i=0;i<100;++i) {
            ...
            common::CombatState c;common::startAttack(a,c,common::AttackKind::Light,{1,0},rulesUnderTest);c.age=15;
    ...
    }
    {
        ...
        for(double seconds:{0.0,.25,1.0,3.0}) {
            auto rules=unstunnedRules();rules.lightWindup=rules.heavyWindup=seconds;rulesUnderTest = rules;
            for(auto kind:{common::AttackKind::Light,common::AttackKind::Heavy}) {
                ...
                check(std::abs(sum-common::attackDescription(kind,rulesUnderTest).startupTicks*common::TICK_DT)<.00001f,"Punch pose must land at configured windup");
                check(client::meleeFrameDuration(kind,frames,9,rulesUnderTest)==.083f,"Recovery frame timing must stay unchanged");
                if(seconds>=1)check(client::meleeFrameDuration(kind,frames,2,rulesUnderTest)>.083f,"Middle windup frames must stretch");
            }
        }
        rulesUnderTest = unstunnedRules();
    }
    {
        ...
        for(auto kind:{common::AttackKind::Light,common::AttackKind::Heavy,common::AttackKind::Explosion,common::AttackKind::Lightning}) {
            ...
            c.age=common::attackDescription(kind,rulesUnderTest).startupTicks-1;
            c.bufferedAttack=common::AttackKind::Light;c.bufferedTicks=10;
            common::applyDamage(p,2,-1,p.pos,rulesUnderTest);
            common::updateAttack(p,c,{&p},empty,{},rulesUnderTest);
    ...
    check(common::startAttack(attacker,combat,common::AttackKind::Light,{},rulesUnderTest),"Light must start");
    check(!common::startAttack(attacker,combat,common::AttackKind::Heavy,{},rulesUnderTest),"No attack during cooldown");
    for(int i=0;i<15;++i)common::updateAttack(attacker,combat,targets,walls,{},rulesUnderTest);
    ...
    for(int i=0;i<48;++i)common::updateAttack(attacker,combat,targets,walls,{},rulesUnderTest);
    ...
    for(int i=0;i<64;++i)common::updateAttack(attacker,combat,targets,walls,{},rulesUnderTest);
    ...
    common::startAttack(attacker,combat,common::AttackKind::Light,{},rulesUnderTest);
    for(int i=0;i<16;++i)common::updateAttack(attacker,combat,targets,open,{},rulesUnderTest);
    check(target.health==0 && !target.alive,"Lethal damage kills and clamps health");
    common::CombatState victim;
    check(!common::startAttack(target,victim,common::AttackKind::Light,{},rulesUnderTest),"Dead players cannot attack");
    for(unsigned i=0;i<common::respawnDelayTicks(rulesUnderTest)-1;++i)
        check(!common::advanceDeath(target,victim,rulesUnderTest),"Death animation must finish before respawn");
    check(common::advanceDeath(target,victim,rulesUnderTest),"Respawn must become ready after 2.5 seconds");
    ...
    check(common::startAttack(target,victim,common::AttackKind::Heavy,{},rulesUnderTest),"Respawned player can attack");
    target.health=-4;
    ...
    check(decoded.combatDebug.attack==common::AttackKind::Heavy && decoded.combatDebug.age==24 &&
    ...
    check(common::startAttack(mouseAttacker,mouseCombat,common::AttackKind::Light,{10,0},rulesUnderTest),"Mouse-aimed attack starts");
    ...
    for(int i=0;i<16;++i)common::updateAttack(mouseAttacker,mouseCombat,mouseTargets,open,{},rulesUnderTest);
    check(mouseTarget.health==80,"Hit follows captured mouse direction despite movement");
    common::CombatState invalid;
    check(!common::startAttack(mouseAttacker,invalid,common::AttackKind::Heavy,{std::numeric_limits<float>::quiet_NaN(),0},rulesUnderTest),"Reject invalid aim");
    invalid.facing={0,1};
    check(common::startAttack(mouseAttacker,invalid,common::AttackKind::Heavy,{},rulesUnderTest),"Cursor at feet has facing fallback");
    check(invalid.attackDirection==sf::Vector2f(0,1),"Zero-length aim fallback");
    common::InputCommand input; input.aim={-0.6f,0.8f}; input.heavyPressed=true;
    ...
    check(common::readInputCmd(inputPacket,id,received) && id==7 && received.aim==input.aim && received.heavyPressed,
    ...
    for(int i=0;i<55;++i) common::updateAttack(buffered,bufferCombat,noTargets,open,{},rulesUnderTest);
    check(common::requestAttack(buffered,bufferCombat,{1,common::AttackKind::Heavy,{0,1},0},rulesUnderTest),"Late click is buffered");
    for(int i=0;i<9;++i) common::updateAttack(buffered,bufferCombat,noTargets,open,{},rulesUnderTest);
    ...
    for(int i=0;i<64;++i) common::updateAttack(buffered,bufferCombat,noTargets,open,{},rulesUnderTest);
    ...
    check(!common::requestAttack(buffered,bufferCombat,{3,common::AttackKind::Light,{1,0},250},rulesUnderTest),"Expired network input cannot start a surprise attack");
    ...
    check(common::inAttackArc({30,29},{1,0},70,common::AttackKind::Light,rulesUnderTest),"Target just inside narrowed cone");
    check(!common::inAttackArc({30,31},{1,0},70,common::AttackKind::Light,rulesUnderTest),"Target outside 90-degree cone misses");
    ...
    check((100+common::attackDescription(common::AttackKind::Light,rulesUnderTest).damage-1)/common::attackDescription(common::AttackKind::Light,rulesUnderTest).damage==5,
          "Five lights should defeat a full-health player");
    check((100+common::attackDescription(common::AttackKind::Heavy,rulesUnderTest).damage-1)/common::attackDescription(common::AttackKind::Heavy,rulesUnderTest).damage==3,
          "Three heavys should defeat a full-health player");
    {
        ...
        check(!common::startAttack(caster,cast,common::AttackKind::Explosion,{501,0},rulesUnderTest),"Spell range must be enforced");
        check(common::startAttack(caster,cast,common::AttackKind::Explosion,{200,0},rulesUnderTest),"Spell did not start");
        for(int i=0;i<23;++i)common::updateAttack(caster,cast,targets,empty,{},rulesUnderTest);
        ...
        check(!common::startAttack(caster,cast,common::AttackKind::Explosion,{200,0},rulesUnderTest),"Spell cooldown bypassed");
        for(int i=24;i<129;++i)common::updateAttack(caster,cast,targets,empty,{},rulesUnderTest);
        check(cast.attack==common::AttackKind::None && a.health==70,"Spell repeated or cooldown never ended");
        caster.pos={200,0};
        check(common::startAttack(caster,cast,common::AttackKind::Explosion,{200,0},rulesUnderTest),"Self-area cast failed");
        for(int i=0;i<24;++i)common::updateAttack(caster,cast,targets,empty,{},rulesUnderTest);
        ...
        cast={};rulesUnderTest.friendlyFire=true;
    ...
    rulesUnderTest = unstunnedRules();
    {
        ...
        check(common::requestAttack(caster,cast,{0,common::AttackKind::Lightning,{200,0},0},rulesUnderTest),"Lightning request rejected");
        for(int i=0;i<15;++i)common::updateAttack(caster,cast,targets,empty,{},rulesUnderTest);
        ...
        for(int i=0;i<21;++i)common::updateAttack(caster,cast,targets,empty,{},rulesUnderTest);
        check(caster.spellSequence==1,"Lightning hit before one third second");
        common::updateAttack(caster,cast,targets,empty,{},rulesUnderTest);check(caster.spellSequence==2,"Lightning repeat missing");
        for(int i=38;i<272;++i)common::updateAttack(caster,cast,targets,empty,{},rulesUnderTest);
        ...
        rulesUnderTest.spell.radius=25;rulesUnderTest.spell.range=250;
        rulesUnderTest.spell.damageMin=5;rulesUnderTest.spell.damageMax=30;
        caster.pos={0,0};victim.pos={200,0};
    ...
    rulesUnderTest = unstunnedRules();
    {
        ...
        common::updateAim(p,c,{0,100},walls,rulesUnderTest);check(c.facing==sf::Vector2f(0,1),"Idle facing must follow cursor");
        common::startAttack(p,c,common::AttackKind::Light,{1,0},rulesUnderTest);
        common::updateAim(p,c,{0,100},walls,rulesUnderTest);check(c.attackDirection==sf::Vector2f(0,1),"Windup aim frozen too early");
        c.age=common::attackDescription(c.attack,rulesUnderTest).startupTicks;
        common::updateAim(p,c,{-100,0},walls,rulesUnderTest);check(c.attackDirection==sf::Vector2f(0,1),"Active swing aim must lock");
        for(auto kind:{common::AttackKind::Explosion,common::AttackKind::Lightning}) {
            c={};common::startAttack(p,c,kind,{100,0},rulesUnderTest);
        ...
        check(!common::spellTargetValid(p,common::AttackKind::Explosion,{100,0},walls,rulesUnderTest),"Wall-blocked cast accepted");
    ...
    {
        common::ServerSettings rules;rulesUnderTest = rules;
        ...
        for(int i=0;i<10;++i)common::updateAttack(p,c,{&p},empty,{},rulesUnderTest);
        common::applyDamage(p,2,-1,p.pos,rulesUnderTest);
        check(p.stunTicks==32,"Repeated damage must reset stun");
        for(int i=0;i<32;++i) {
            check(!common::startAttack(p,c,common::AttackKind::Light,{1,0},rulesUnderTest),"Attacks must be blocked during stun");
            common::updateAttack(p,c,{&p},empty,{},rulesUnderTest);
        }
        check(common::startAttack(p,c,common::AttackKind::Light,{1,0},rulesUnderTest),"Attacks must resume when stun ends");
        rules.damageStunSeconds=.25;rulesUnderTest = rules;
        common::applyDamage(p,1,-1,p.pos,rulesUnderTest);check(p.stunTicks==16,"Custom damage stun ignored");
    ...
    {
        auto rules=unstunnedRules();rules.respawnSeconds=.25;rulesUnderTest = rules;
        common::PlayerState p;p.alive=false;p.health=0;common::CombatState c;
        for(int i=0;i<15;++i)check(!common::advanceDeath(p,c,rulesUnderTest),"Custom respawn fired too early");
    ...
    }
    {
        ...
        check(common::startAttack(p,c,common::AttackKind::Explosion,{200,0},rulesUnderTest),"Explosion starts");
        for(int i=0;i<25;++i)common::updateAttack(p,c,{&p},empty,{},rulesUnderTest);
        check(c.attack==common::AttackKind::None && p.explosionCooldown>0,"Explosion must enter its own cooldown");
        check(!common::startAttack(p,c,common::AttackKind::Explosion,{200,0},rulesUnderTest),"Explosion cooldown must block explosion");
        check(common::startAttack(p,c,common::AttackKind::Lightning,{200,0},rulesUnderTest),"Explosion cooldown must allow lightning");
