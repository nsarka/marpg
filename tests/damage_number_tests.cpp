...
int main(){
    ...
    for(auto& p:players)p.connected=true;
    ...
    check(incoming.amount==2 && incoming.incoming && DamageNumbers::color(incoming).r>200,"Incoming damage is red");
    check(outgoing.amount==10 && !outgoing.incoming && DamageNumbers::color(outgoing)==sf::Color::White,"Outgoing damage is white");
    check(DamageNumbers::label(incoming)=="-2" && DamageNumbers::label(outgoing)=="-10", "All damage numbers show health loss with a minus sign");
    check(outgoing.origin.x>=88 && outgoing.origin.x<=112 && outgoing.origin.y>=180 && outgoing.origin.y<=192,
    ...
    numbers.observe(players,0,false);
    check(numbers.numbers().size()==3 && numbers.numbers()[2].origin!=outgoing.origin,"Separate hits get their own random locations");
    numbers.update(0.45f);
    check(DamageNumbers::position(numbers.numbers()[1]).y<outgoing.origin.y &&
          DamageNumbers::color(numbers.numbers()[1]).a<150,"Numbers rise and fade");
    ...
    check(allCombat.numbers().back().amount==15 && DamageNumbers::color(allCombat.numbers().back())==sf::Color::White,"Third-party combat must be white");
    ...
    std::cout<<"PASS: attribution, colors, contact offset, per-hit variation, deduplication, rise and fade\n";
}
