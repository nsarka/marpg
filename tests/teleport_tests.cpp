...
int main(int argc, char** argv) {
    ...
    common::TeleportSystem actual(registry);
    actual.load(argv[1]);
    common::TriggerSystem unified;
    unified.load(argv[1]);
    ...
    std::ofstream(fixture)
        << R"(<map version="1.10" orientation="isometric" width="20" height="20" tilewidth="128" tileheight="64">
    ...
    check(registry->all().size() == 2, "Portal reload retained stale debug geometry");
    {
        ...
        check(registry->all().size() == 3 && registry->all().back() == futureTrigger,
              "New trigger kind requires debug registration");
    ...
    check((p.pos - b).length() < .01f && p.vel == sf::Vector2f{} && p.teleportSequence == 1,
          "Wrong portal destination");
    for (int i = 0; i < 100; ++i)
        check(!actual.update(0, p, walls), "Arrival teleported back immediately");
    p.pos = {-10000, -10000};
    actual.update(0, p, walls);
    p.pos = b;
    check(actual.update(0, p, walls) && (p.pos - a).length() < .01f, "Return portal failed after leaving");
    common::PlayerState other;
    ...
    p.alive = true;
    walls.addPolygon({b + sf::Vector2f{-20, -20}, b + sf::Vector2f{20, -20}, b + sf::Vector2f{20, 20},
                      b + sf::Vector2f{-20, 20}});
    ...
    check(cues.size() == 1 && cues[0].effect == SoundEffect::Teleport, "Teleport sound missing");
    check(events.observe(players).empty(), "Teleport sound repeated");
    sf::Packet packet;
    common::writeWorldPacket(packet, players);
    std::string type;
    packet >> type;
    std::vector<common::PlayerState> decoded;
    check(common::readWorldPacket(packet, decoded), "Teleport snapshot failed");
    ...
    std::string damageMap((std::istreambuf_iterator<char>(fixtureInput)), {});
    fixtureInput.close();
    damageMap.insert(
        damageMap.find("</objectgroup>"),
        R"(<object id="3" name="Fantasy hazard" type="DamageTrigger" x="190" y="190" width="20" height="20"/>)");
    std::string floor = "<layer name=\"Floor\" width=\"20\" height=\"20\"><data encoding=\"csv\">";
    for (int i = 0; i < 400; ++i)
        floor += (i ? ",1" : "1");
    floor += "</data></layer>";
    damageMap.insert(damageMap.find("</map>"), floor);
    std::ofstream(fixture) << damageMap;
    common::ServerSettings rules;
    rules.triggerDamage = 7;
    rules.triggerIntervalSeconds = .5;
    common::TriggerSystem hazards;
    hazards.load(fixture.string(), rules);
    check(hazards.regions().all().size() == 3, "Object damage region missing from shared debug registry");
    common::PlayerState victim;
    victim.connected = true;
    victim.pos = project(200, 200);
    ...
    hazards.update(0, victim);
    check(victim.health == 86, "Object-layer damage did not repeat");
    victim.pos = project(250, 250);
    hazards.update(0, victim);
