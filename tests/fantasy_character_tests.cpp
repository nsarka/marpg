...
int main(int argc,char** argv) {
    ...
    for(const auto* name:common::CharacterNames) {
        ...
        check(characterResources.loadFantasyCharacter(name,root/"assets/Fantasy tileset - 2D Isometric/Characters"/name),"Team character failed to load");
        for(std::size_t i=0;i<common::CharacterAnimations.size();++i)
            for(const auto& clip:characterResources.getCharacterAnimations(name).get(static_cast<common::CharacterAnimation>(i)).byFacing)
    ...
    {
        PlayerAnimation animation;animation.setAnimations(animations);
        using F=PlayerAnimation::Facing8;using A=common::CharacterAnimation;
        for(const auto& direction:std::array<std::pair<sf::Vector2f,F>,4>{{{{1,0},F::Dir6},{{0,1},F::Dir8},{{-1,0},F::Dir2},{{0,-1},F::Dir4}}}){
            animation.face(direction.first);check(animation.facing()==direction.second,"Facing changed during controller extraction");
        }
        common::ServerSettings settings;settings.lightWindup=1;
        common::PlayerState previous,state;previous.connected=state.connected=true;
        state.lastAttack=common::AttackKind::Light;state.attackSequence=1;
        animation.observe(previous,state);animation.update(.5f,state,settings,false,50,140);
        check(animation.current()==A::Attack1 && animation.frame()<7,"Controller ignored configured windup");
        previous=state;state.health-=2;animation.observe(previous,state);
        check(animation.current()==A::TakeDamage,"Damage did not interrupt attack animation");
        previous=state;state.alive=false;state.health=0;animation.observe(previous,state);
        check(animation.current()==A::Die,"Death animation missing");
        previous=state;state.alive=true;state.health=100;animation.observe(previous,state);
        check(animation.current()==A::Idle && animation.frame()==0,"Respawn did not reset animation");
        previous=state;state.lastAttack=common::AttackKind::Explosion;++state.attackSequence;animation.observe(previous,state);
    ...
    check(!animations.get(A::Die).byFacing[0].looping && !animations.get(A::TakeDamage).byFacing[0].looping,"Death or hurt loops");
    check(animations.get(A::Attack1).byFacing[0].texture!=animations.get(A::Attack4).byFacing[0].texture,"Melee swings share a sheet");
